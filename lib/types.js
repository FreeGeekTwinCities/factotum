...
exports.transaction = new Type('transaction', {
    fields: {
        ...
        items: fields.array({
            /* the parseEach function should split off the numeric part of each item and use that as the item quantity */
            parseEach: function (string) {
                return {quantity: parseFloat(string.split(" ",1)[0]), description: string.slice(string.indexOf(" "),string.length)};
            }
        }),
        payments: fields.array({required: false}),
        comments: fields.string({required: false, widget: widgets.textarea({cols: 40, rows: 10})}),
        date: {
            year: fields.string({default_value: function (req) {return new Date().getFullYear();}}),
            month: fields.string({default_value: function (req) {return new Date().getMonth();}, hint: "NOTE: Months are zero-indexed, so January=0, February=1, etc."}),
            day: fields.string({default_value: function (req) {return new Date().getDate();}})
        },
        audit: {
...
exports.timesheet_line = new Type('timesheet_line', {
    fields: {
	    ...
	    audit: {
            creator: fields.creator({required: false}),
            createdTime: fields.createdTime({required: false})
        }
    }
});
